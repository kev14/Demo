	...
	var sb = PBS.KIDS.storybook,
		...
		onNavigateComplete = function () {
			...
			if (targetPageIndex === -1) {
				...
				if (curOrientation === "SINGLE-PAGE") {
					...
					if (leftPageIndex === curPageIndex) {
						if (pages[leftPageIndex].pageSound && GLOBAL.speechState === true) {
							// Play the page sound
							audioPlayer.play(pages[leftPageIndex].pageSound);
						}
					// If the page is the right page
					} else {
						if (pages[rightPageIndex].pageSound && GLOBAL.speechState === true) {
							// Play the page sound
							audioPlayer.play(pages[rightPageIndex].pageSound);
						}
					}
				// if two-page layout
					...
					} else if (pages[rightPageIndex].pageSound && GLOBAL.speechState === true ) {
						// Play the page sound
						loud();
						audioPlayer.play(pages[leftPageIndex].pageSound);
						audioPlayer.addEventListener("PLAY_COMPLETE", rightPageSoundComplete);
			...
			if (targetPageIndex === -1) {
				...
				//turn off read button after cover page
				if (curOrientation === "SINGLE-PAGE") {
					if(readButtonElement){
		...
		start = function () {
			...
			if(readMyselfButtonSprite){
				...
				readMyselfButtonSprite.addEventListener("PRESS",that.readMyself);
					console.log(readMyselfButtonSprite);
			}
		...
		init = function () {
			...
			if (!initialized) {
				...
				if (bookConfig.readButton) {
					...
					readButtonElement.className = "pbsPageContainer";
				...
				if (bookConfig.readMyselfButton) {
		           ...
		           readMyselfButtonElement.id = "pbsReadMyselfButton";
					readMyselfButtonElement.className = "pbsPageContainer";
